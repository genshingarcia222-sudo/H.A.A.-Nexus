...
| A4 | ~~`tauri dev` not yet run; runtime IPC unexercised~~ **PARTLY CLEARED** — `tauri dev` now runs: the webview launches and real `invoke` calls round-trip through Rust and SQLite (fixing a Vite watcher defect that had made `tauri dev` fail outright on Windows). `tauri build` / installer packaging remains unverified (Phase 9). See CHANGELOG, "Runtime IPC Verified in the Real Tauri Shell" |
