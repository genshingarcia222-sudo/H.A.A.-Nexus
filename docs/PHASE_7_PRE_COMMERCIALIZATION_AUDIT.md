...
| A4 | ~~`tauri dev` not yet run; runtime IPC unexercised~~ **PARTLY CLEARED** — `tauri dev` now runs: the webview launches and real `invoke` calls round-trip through Rust and SQLite (fixing a Vite watcher defect that had made `tauri dev` fail outright on Windows). the release build also compiles and runs standalone (`tauri build`); only MSI/NSIS **bundling** remains unverified, having failed on a DNS error while downloading the WiX toolset (Phase 9). See CHANGELOG, "Runtime IPC Verified in the Real Tauri Shell" |
